...
public class CliqueFinder {
    ...
    /**
     * Code was copied from Kripo yasara plugin written by Dave Wood, August 8th 2008 and was translated from Python to
     * Java/ejml/oo
     * 
     * @throws NoOverlapFoundException
     */
    ...
    /**
     * BronKerbosch clique detection algorithm Based on the pseudo code in 'Reporting maximal cliques: new insights into an old
     * problem' Cazaals and Chinmay 2007
     * 
     * @param potential_clique
     * @param candidates
     * @param already_found
     */
    private void findCliques(List<PointPair> potential_clique, List<PointPair> candidates, List<PointPair> already_found) {
        ...
        if (searchComplete) {
            ...
            boolean moreTypes = true;
            if (!bestCliques.isEmpty()) {
                List<PointPair> bestClique = bestCliques.peek();
                larger = potential_clique.size() > bestClique.size();
                sameSize = potential_clique.size() == bestClique.size();
                moreTypes = nrTypes(potential_clique) > nrTypes(bestClique);
            }
            // RECORD CLIQUE IF IT IS THE LARGEST FOUND SO FAR OR (IS SAME SIZE AND HAS MORE TYPES)
            if (bestCliques.isEmpty() || larger || (sameSize && moreTypes)) {
                List<PointPair> bestClique = potential_clique;
                if (!bestCliques.offerFirst(bestClique)) {
                    // bestCliques is over capacity, make room for best clique yet, by dropping worst clique
            ...
            while (!candidates.isEmpty()) {
                ...
                while (j < p_new.size()) {
                    ...
                    if ((!pairs.containsKey(up_i)) || (!(pairs.get(up_i).contains(new PointPair(u_i.second, p_i.second))))) {
                        p_new.remove(j);
                        j--;
                    }
                    j++;
                ...
                while (j < x_new.size()) {
                    ...
                    if ((!pairs.containsKey(ux_i)) || (!(pairs.get(ux_i).contains(new PointPair(u_i.second, x_i.second))))) {
                        x_new.remove(j);
                        j--;
                    }
                    j++;
    ...
    private int nrTypes(List<PointPair> clique) {
        Set<String> types = new HashSet<>();
        for (PointPair pp : clique) {
            types.add(reference.get(pp.first).type);
        }
        return types.size();
    }
